...
from utils.util_diffusion import checkpoint
...
class BasicTransformerBlock_21(nn.Module):
    ATTENTION_MODES = {
...
class SpatialTransformer_21(nn.Module):
    """
    Transformer block for image-like data.
    First, project the input (aka embedding)
    and reshape to b, t, d.
...
class BasicTransformerBlock3D(BasicTransformerBlock_21):

    def forward(self, x, context=None, num_frames=1):
        return checkpoint(self._forward, (x, context, num_frames),
                          self.parameters(), self.checkpoint)

    def _forward(self, x, context=None, num_frames=1):

        num_frames = int(num_frames.item()) if isinstance(
            num_frames, torch.Tensor) else num_frames
